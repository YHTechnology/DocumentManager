...
namespace FileHelper
{
    public class HttpFileUploader : IFileUploader
    {
        ...
        private void StartUpload()
        {
            ...
            bool isLastChunk = dataToSend <= (1024 * 4096);
            bool isFirstChunk = _dataSent == 0;

            UriBuilder httpHandlerUrlBuilder = new UriBuilder(UploadUrl);
            httpHandlerUrlBuilder.Query
                = string.Format("{5}file={0}&offset={1}&last={2}&first={3}&param={4}"
                                , HttpUtility.UrlEncode(_file.FileName)
                                , _dataSent
                                , isLastChunk
                                , isFirstChunk
                                , _initParams
